...
class TDI_Helpers {
		...
		if ( empty( $import_file_info['import_file_url'] ) ) {
			if ( file_exists( $import_file_info['local_import_file'] ) ) {
				...
				return new WP_Error(
					...
					sprintf(
						__( '"import_file_url" or "local_import_file" for %s%s%s are not defined!', 'minisite-lite' ),
						'<strong>',
	...
	private static function get_content_from_url( $url, $file_name = 'Import file' ) {
		...
		if ( empty( $url ) ) {
			return new WP_Error(
				'url_not_defined',
				sprintf(
					__( 'URL for %s%s%s file is not defined!', 'minisite-lite' ),
					'<strong>',
					$file_name,
					'</strong>'
				)
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new WP_Error(
				'file_fetching_error',
				sprintf(
					__( 'An error occurred while fetching %s%s%s file from the server!%sReason: %s - %s.', 'minisite-lite' ),
					'<strong>',
					$file_name,
					'</strong>',
					'<br>',
	...
	public static function write_to_file( $content, $file_path ) {
		...
		if ( ! $wp_filesystem->put_contents( $file_path, $content ) ) {
			return new WP_Error(
				...
				sprintf(
					__( 'An error occurred while writing file to your server! Tried to write a file to: %s%s.', 'minisite-lite' ),
					'<br>',
					$file_path
				)
			);
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new WP_Error(
				...
				sprintf(
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %s%s.', 'minisite-lite' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( ! ( 'direct' === get_filesystem_method() ) ) {
			return new WP_Error(
				...
				sprintf(
					__( 'This WordPress page does not have %sdirect%s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %s.', 'minisite-lite' ),
					'<strong>',
					'</strong>',
					'<a href="" target="_blank">How to set <strong>direct</strong> filesystem method</a>'
				)
			);
		}
		...
		$plugin_page_setup = apply_filters( 'theme-demo-import/plugin_page_setup', array(
				...
				'menu_title'  => esc_html__( 'Import Demo Data' , 'minisite-lite' ),
				'capability'  => 'import',
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
			return new WP_error(
				...
				__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'minisite-lite' )
			);
		}
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'minisite-lite' )
			);
		}
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$attachment = array(
			...
			'post_title'     => apply_filters( 'theme-demo-import/attachment_prefix', esc_html__( 'Theme Demo Import - ', 'minisite-lite' ) ) . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
	...
	public static function verify_ajax_call() {
		...
		if ( ! current_user_can( 'import' ) ) {
			wp_die(
				sprintf(
					__( '%sYour user role isn\'t high enough. You don\'t have permission to import demo data.%s', 'minisite-lite' ),
	...
	public static function process_uploaded_files( $uploaded_files, $log_file_path ) {
		...
		if ( empty( $content_file_info['file'] ) || isset( $content_file_info['error'] ) ) {
			...
			self::log_error_and_send_ajax_response(
				__( 'Please upload XML file for content import. If you want to import widgets or customizer settings only, please use Widget Importer & Exporter or the Customizer Export/Import plugin.', 'minisite-lite' ),
				$log_file_path,
				esc_html__( 'Upload files', 'minisite-lite' )
			);
		}
		...
		if ( $widget_file_info && ! isset( $widget_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Widget file was not uploaded. Error: %s', 'minisite-lite' ),
		...
		if ( $customizer_file_info && ! isset( $customizer_file_info['error'] ) ) {
			...
			$log_added = self::append_to_file(
				sprintf(
					__( 'Customizer file was not uploaded. Error: %s', 'minisite-lite' ),
		...
		$log_added = self::append_to_file(
			__( 'The import files were successfully uploaded!', 'minisite-lite' ) . self::import_file_info( $selected_import_files ),
			$log_file_path,
			esc_html__( 'Upload files' , 'minisite-lite' )
		);
	...
	public static function import_file_info( $selected_import_files ) {
		return PHP_EOL .
		sprintf(
			__( 'Initial max execution time = %s', 'minisite-lite' ),
			ini_get( 'max_execution_time' )
		) . PHP_EOL .
		sprintf(
			__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s', 'minisite-lite' ),
			PHP_EOL,
			get_site_url(),
			$selected_import_files['content'],
			empty( $selected_import_files['widgets'] ) ? esc_html__( 'not defined!', 'minisite-lite' ) : $selected_import_files['widgets'],
			empty( $selected_import_files['customizer'] ) ? esc_html__( 'not defined!', 'minisite-lite' ) : $selected_import_files['customizer']
		);
	}
